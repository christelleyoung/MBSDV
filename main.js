...
d3.json(data_url, function(nations){
...
//    canvas.attr("transform", "translate(" + margin.left + "," +margin.top ")");
    canvas.attr("transform", "translate(50, 10)");

//    var xScale = d3.scale.log(); //for the income
//    xScale.domain([250,1e10]); // min and max data value
//    xScale.range([0, canvas_width]); //min and max position on the pa

    var xScale =d3.scale.log().domain([250,1e5]).range([0, canvas_width]); // Same line of code to above
    var xAxisGenerator =d3.svg.axis().scale(xScale).orient('bottom');

    canvas.append("g")
        .attr("class", " x axis")
        .attr("transform", "translate(0,"+canvas_height+")") // if we do not do this it will start at 0,0
        .call(xAxisGenerator)

    var yScale =d3.scale.linear().domain([10,85]).range([canvas_height,0]); // Same line of code to above
//    var colorScale = d3.scale.category10();
//   var colorScale = ["#CCAAEE", "#CCEEAA", "#2ca02c", "#d62728", "#9467bd",
//                       "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf", "#CCAAEE", "#CCEEAA", "#2ca02c", "#d62728", "#9467bd",
//                       "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf","#CCAAEE", "#CCEEAA", "#2ca02c", "#d62728", "#9467bd",
//                       "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf"];
    var colors = ['#762a83','#af8dc3','#e7d4e8','#d9f0d3','#7fbf7b','#1b7837']; //define the colors in an array
    var colorScale = d3.scale.ordinal().range(colors);  //then use colorScale as any other scale

    var yAxisGenerator =d3.svg.axis().scale(yScale).orient('left');
    canvas.append("g")
        .attr("class", " y axis")
        .call(yAxisGenerator)
    ...
    //
    //    circle.attr('{cx:25; cy:25; r:15; fill:"pink"; fill-opacity:"0.7";}');
    }
    updatePlot();
